...
import { rollD20, resolveRoll, performDiceRoll, isStandardDieSides } from '../src/session-engine/dice';
import {
...
async function emitRoomState(sessionId: string) {
  ...
  io.to(roomChannel(sessionId)).emit('room:state', sanitizeStateForPlayers(state));
  io.to(hostChannel(sessionId)).emit('room:state', state);
}

io.on('connection', (socket: Socket) => {
  socket.on('room:join', async (payload: JoinPayload, ack?: (result: unknown) => void) => {
    try {
  ...
  socket.on('host:action', async (payload: HostActionPayload, ack?: (result: unknown) => void) => {
    try {
      await getLiveSessionForHost(payload.sessionId, payload.hostId);

      if (payload.event.type === 'session_started') {
        const record = await startLiveSession(payload.sessionId, payload.hostId);
        ...
        const record = await appendSessionEvent(payload.sessionId, payload.event);
        ...
        const rollEvent: RoomEventPayload = {
          type: 'dice_roll',
          roll: buildDiceRollPayload(payload.hostId, '', 'host', sides, count),
        };
  ...
  socket.on(
    'player:lobby_action',
    async (payload: PlayerLobbyActionPayload, ack?: (result: unknown) => void) => {
      try {
        if (!(await verifyPlayerInSession(payload.sessionId, payload.playerId))) {
          ack?.({ ok: false, error: 'Invalid player' });
          return;
        }
        ...
        if (payload.action === 'claim_slot') {
          ...
          await claimHeroSlot(payload.sessionId, payload.playerId, payload.heroSlotId);
  ...
  socket.on('player:dice_roll', async (payload: PlayerDiceRollPayload, ack?: (result: unknown) => void) => {
    try {
      if (!(await verifyPlayerInSession(payload.sessionId, payload.playerId))) {
        ack?.({ ok: false, error: 'Invalid player' });
        return;
  ...
  socket.on('player:roll', async (payload: PlayerRollPayload, ack?: (result: unknown) => void) => {
    try {
      const state = await getRoomState(payload.sessionId);
      if (!state?.pendingRoll || state.pendingRoll.playerId !== payload.playerId) {
