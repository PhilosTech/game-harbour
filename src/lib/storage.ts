...
function normalizeS3Endpoint(endpoint: string, bucket: string): string {
  ...
  if (
    trimmed.includes(bucketHostPrefix) &&
...
function validateStorageEndpoint(
  endpoint: string,
  ...
  if (
    publicUrl &&
    endpoint.replace(/\/$/, "") === publicUrl.replace(/\/$/, "")
  ) {
    throw new StorageConfigError(
  ...
  if (isR2Endpoint(endpoint)) {
    try {
      ...
      if (!host.endsWith(".r2.cloudflarestorage.com")) {
        throw new StorageConfigError(
          "S3_ENDPOINT host must end with .r2.cloudflarestorage.com",
        );
      }
    } catch {
...
export function getStorageDiagnostics(): StorageDiagnostics {
  ...
  if (!endpoint) {
    ...
    warnings.push(
      "S3_PUBLIC_URL looks like R2 but S3_ENDPOINT is not an r2.cloudflarestorage.com host",
    );
  }
  ...
  if (
    !sanitizeEnv(process.env.S3_ACCESS_KEY_ID) ||
    !sanitizeEnv(process.env.S3_SECRET_ACCESS_KEY)
  ) {
