...
import { notFound, redirect } from 'next/navigation';import {
...
export default async function BridgeSessionPage({ params }: Props) {
  ...
  try {
    const liveSession = await getLiveSessionForHost(sessionId, authSession.user.id);
    const scenes = await getSessionScenesForHost(sessionId, authSession.user.id);
