...
const DiceRollSceneClient = dynamic(
  () => import('@/components/shared/dice-roll-scene').then((mod) => mod.DiceRollScene),
  { ssr: false },
);
...
export function DiceRollOverlay({ roll, isHost, onDismiss }: DiceRollOverlayProps) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex flex-col bg-black/85 backdrop-blur-sm">
      ...
      >
        <DiceRollSceneClient
          rollId={roll.id}
          notation={buildPredeterminedDiceNotation(roll.count, roll.sides, roll.values)}
      ...
      <div className="relative z-10 shrink-0 border-t border-white/10 bg-card/95 px-4 py-4">
        {!isRolling ? (
          <div className="mb-4 space-y-1 text-center">
            {roll.count > 1 ? (
              ...
              <p className="text-4xl font-bold tabular-nums text-foreground">{roll.total}</p>
            )}
          </div>
        ) : (
        ...
        {isHost && !isRolling && onDismiss ? (
          ...
          <p className="text-center text-xs text-muted">{t('waitingHostDismiss')}</p>
        ) : (
