...
export function LoginForm() {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4 rounded-2xl border border-border bg-card p-5">
      ...
      {resetSuccess ? (
        <p className="rounded-xl border border-emerald-900/50 bg-emerald-950/40 px-3 py-2 text-sm text-emerald-200" role="status">
          {t('resetSuccess')}
        </p>
      ) : null}
      ...
      <div className="space-y-2 text-center text-sm text-muted">
        <p>
          <Link href={`/${locale}/bridge/reset-password`} className="underline hover:text-foreground">
            {t('forgotPassword')}
          </Link>
        </p>
        <p>
          <Link href={`/${locale}/bridge/register`} className="underline hover:text-foreground">
            {t('createAccount')}
          </Link>
        </p>
      </div>
