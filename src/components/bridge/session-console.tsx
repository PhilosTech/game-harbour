...
export function SessionConsole({
  ...
  const { roomState, isConnected, error, emitHostAction, submitDiceRoll, dismissDiceRoll } =
    useRoomSocket({
      role: 'host',
      sessionId,
      hostId,
  ...
  const readyCount =
    roomState?.players.filter((player) => player.character?.isReady).length ?? 0;
  ...
  return (
    <div className="space-y-6">
      ...
      {phase === 'ACTIVE' ? (
        <section className={`${panelClass} border-accent/20 bg-gradient-to-br from-card to-accent/5`}>
