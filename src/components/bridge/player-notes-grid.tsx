...
export function PlayerNotesGrid({ scenes, value, onCellChange }: PlayerNotesGridProps) {
  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-border/80 bg-background/40">
      <table className="w-full min-w-[240px] border-collapse text-xs">
        ...
        <tbody>
          {Array.from({ length: PLAYER_NOTES_DATA_ROW_COUNT }, (_, rowIndex) => (
              ...
              {scenes.map((scene, sceneIndex) => (
                <td key={scene.id} className="p-1">
                  <input
                    ...
                    onChange={(event) => onCellChange(rowIndex, sceneIndex, event.target.value)}
